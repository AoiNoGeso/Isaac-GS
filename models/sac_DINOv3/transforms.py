...
class DINOTransform:
    ...
    rgbがスタック観測(3*Nチャンネル)の場合は各フレームを1回のバッチforwardでまとめて処理する"""
    ...
    def transform_observation_space(self, obs_space: spaces.Dict) -> spaces.Dict:
        ...
        stack_size = obs_space["rgb"].shape[0] // 3
        dino_feat_space = spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(
                self._backbone.hidden_size * stack_size,
                self._backbone.grid_size,
                self._backbone.grid_size,
            ),
            dtype=np.float32,
        )
    ...
    def __call__(self, obs: dict) -> dict:
        ...
        obs = dict(obs)
        obs["dino_feat"] = self._backbone.extract_stack(obs["rgb"])
        del obs["rgb"]
        return obs
